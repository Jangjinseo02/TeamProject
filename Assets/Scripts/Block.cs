...
public class Block : MonoBehaviour
{
    ...
    private void Start()
    {
        afVec = transform.localPosition;
        pos = afVec;
    }

    public void MoveCheck()
    {
        if (afVec.y - transform.localPosition.y >= 1)
        {
            isMove = true;
        }
    }
    ...
    IEnumerator GetDropEnumerator(float gravity)
    {
        ...
        while (true)
        {
            ...
            //if (isMove)
            //{
            //    this.row = Mathf.CeilToInt(transform.localPosition.y);
            //    transform.localPosition = new Vector3(col, row, 0);
            //    //group.blockManager.Collision(this);
